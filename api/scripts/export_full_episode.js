...
async function exportFullEpisode() {
  ...
  try {
    ...
    console.log(`[Export] Found ${users.length} users. Beginning export loop...`);

    for (const user of users) {
      const userId = user._id;
      const userName = user.name || user.username || user.email || user.guid || userId.toString();
      const safeName = sanitizeFilename(userName);
      
      console.log(`\n======================================================`);
      console.log(`[Export] Processing User: ${userName} (${userId})`);
      
      let startTime = null;
      let endTime = null;

      // 1. Try to find an EpisodeAnalysis
      console.log(`[Export][${safeName}] Searching for EpisodeAnalysis...`);
      ...
      if (episodes.length > 0) {
        ...
        console.log(`[Export][${safeName}] Found EpisodeAnalysis. Start: ${new Date(startTime)}, End: ${new Date(endTime)}`);
      } else {
        console.log(`[Export][${safeName}] No EpisodeAnalysis found. Searching for AnomalyEvent...`);
        ...
        if (anomalyEvents.length > 0) {
          ...
          console.log(`[Export][${safeName}] Found AnomalyEvent. Start: ${new Date(startTime)}, End: ${new Date(endTime)}`);
        } else {
          console.log(`[Export][${safeName}] No AnomalyEvent found. Searching for Segments...`);
          ...
          if (lastSegment.length > 0) {
            ...
            console.log(`[Export][${safeName}] Found Segments. Using last 30 minutes block. Start: ${new Date(startTime)}, End: ${new Date(endTime)}`);
          } else {
            console.log(`[Export][${safeName}] No data found for this user. Skipping...`);
            continue;
          }
        }
      }
      ...
      console.log(`[Export][${safeName}] Fetching data between ranges...`);
      const segments = await db.collection('segments').find({
        user_id: userId,
        window_start: { $gte: startTime, $lte: endTime }
      }).sort({ window_start: 1 }).toArray();
      
      console.log(`[Export][${safeName}] Found ${segments.length} segments.`);
      ...
      console.log(`[Export][${safeName}] Found ${rawData.length} raw data points.`);
      ...
      console.log(`[Export][${safeName}] Found ${episodeAnalysis.length} episode analyses.`);
      ...
      console.log(`[Export][${safeName}] Creating Excel workbook...`);
      ...
      sheetMarkov.columns = [
        ...
        { header: 'Transitions (State : Prob : Count)', key: 'transitions', width: 80 },
      ];
      
      if (markovModels.length === 0) {
        sheetMarkov.addRow({
        ...
        markovModels.forEach(m => {
            ...
            matrix.forEach(row => {
                const transStrings = row.transitions.map(t => {
                   if (t.probability === null) return null;
                   return `${t.next_state} (P=${t.probability.toFixed(2)}, C=${t.count})`;
                }).filter(Boolean);

                sheetMarkov.addRow({
                    status: m.status || 'UNKNOWN',
                    state: row.current_state,
                    transitions: transStrings.join(' | ')
                });
            });
        });
      }

      const outputPath = path.resolve(process.cwd(), `episode_${safeName}.xlsx`);
      await workbook.xlsx.writeFile(outputPath);
      console.log(`[Export][${safeName}] Success! Data exported to: ${outputPath}`);
    }
