/**
 * scripts/test_baseline_window_optimization.js
 *
 ...
 * Murni membaca data Segment/Baseline untuk komparasi statistik & simulasi.
 */

import mongoose from 'mongoose';
import dotenv from 'dotenv';
import Segment from '../api/models/segment.model.js';
import Baseline from '../api/models/baseline.model.js';
...
export async function runWindowOptimizationSimulation(customUserId = null) {
  try {
    ...
    // Ambil segmen dari database untuk simulasi
    ...
    if (!segments || segments.length === 0) {
      return {
        success: false,
        message: 'Tidak ada data segmen teranalisis untuk simulasi. Memerlukan data segmen di database.',
    ...
    let totalSamples30 = 0;
    let totalSamples15 = 0;
    let sumFprShift = 0;

    for (const [groupKey, segList] of Object.entries(grouped)) {
      if (segList.length < 15) continue; // Butuh minimal 15 segmen untuk simulasi
      ...
      // Simulasi Klasifikasi Anomali pada sisa segmen uji (Holdout Evaluation)
      ...
      for (const seg of holdoutSegs) {
        ...
        // Anomaly Z-Score threshold test (Z > 2.0 = Caution)
      ...
      totalSamples30 += sample30_HR.length;
      totalSamples15 += sample15_HR.length;
      sumFprShift += (fpr15 - fpr30);
    ...
    const report = {
      ...
      evaluation_summary: {
        ...
        recommendation: {
          ...
          detail_indonesian: "Mengubah syarat baseline awal menjadi 15 window (30 menit total) SANGAT LAYAK dan LEBIH OPTIMAL untuk efisiensi partisipan. Untuk menjaga akurasi medis, gunakan 15 window sebagai 'Provisional Baseline' (langsung aktif untuk deteksi live), dan biarkan sistem di background terus melengkapi hingga 30 window untuk 'Mature Baseline'.",
          action_plan: [
...
if (process.argv[1] && process.argv[1].includes('test_baseline_window_optimization.js')) {
