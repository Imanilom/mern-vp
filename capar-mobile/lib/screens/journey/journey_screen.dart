...
class _JourneyScreenState extends State<JourneyScreen> {
  ...
  Future<void> _loadBaselines() async {
    ...
    if (uid.isNotEmpty) {
      try {
        final result = await ApiService.getUserBaselines(uid);
        if (mounted && result != null && result['data'] != null) {
          setState(() {
            baselines = result['data'] as List<dynamic>;
            isLoading = false;
          });
        }
      } catch (e) {
        debugPrint('Error loading baselines: $e');
      }
    }
                  ...
                  children: [
                    ...
                    if (isLoading)
                      ...
                      const Padding(
                        ...
                        child: Center(child: Text('Belum ada baseline aktif', style: TextStyle(color: AppColors.gray))),
                      )
                    else
                      ...baselines.map((b) {
                         double pct = 0.0;
                         if (b['stats']?['mean_rr'] != null) pct = 0.85; // Just a mock for demo since real fraction requires maturity
                         if (b['is_mature'] == true) pct = 1.0;
                         String statusLabel = (b['is_mature'] == true) ? 'READY' : 'LEARNING';
                         Color color = (b['is_mature'] == true) ? AppColors.green : AppColors.teal;
                         return Padding(
                           padding: const EdgeInsets.only(bottom: 12.0),
                           child: _buildContextJourneyRow(b['activity'] ?? 'Unknown', '${(pct * 100).toInt()}%', statusLabel, pct, color),
                         );
                      }),
                  ],
                ),
